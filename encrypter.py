...
def encrypter():
    ...
    files_to_encrypt = sorted(tools.list_dir('files'))
    
    # Calculate checksum of the original file (stored in uploads)
    original_file_name = tools.list_dir('uploads')[0]
    checksum = get_file_checksum('./uploads/' + original_file_name)
    
    for index in range(0, len(files_to_encrypt)):
        if index % 4 == 0:
            AESAlgoRotated(files_to_encrypt[index], key_1_1, key_1_2)
        elif index % 4 == 1:
            ChaChaAlgo(files_to_encrypt[index], key_2, nonce12)
        elif index % 4 == 2:
            AESGCMAlgo(files_to_encrypt[index], key_3, nonce12)
        else:
            AESCCMAlgo(files_to_encrypt[index], key_4, nonce13)
            
    secret_information = (checksum.encode())+b":::::"+(key_1_1)+b":::::"+(key_1_2)+b":::::"+(key_2) + \
        b":::::"+(key_3)+b":::::"+(key_4)+b":::::" + \
